...
export class MatchPageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      // load auth info from localStorage if present
      const stored = localStorage.getItem(`match_${id}_auth`);
      if (stored) {
        try {
          const parsed = JSON.parse(stored);
          this.role = parsed.role;
          this.myTeamIndex = parsed.team;
          this.captainToken = parsed.token;
        } catch {
          // ignore parse errors
        }
      }
